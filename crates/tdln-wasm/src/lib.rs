...
pub fn run(bytes: &[u8], _input: &Value, fuel_limit: u64) -> Result<ExecOutcome> {
    ...
    let func = instance.get_func(&mut store, "run").ok_or_else(|| anyhow!("missing_export: run"))?;
    let typed = func.typed::<(), i32>(&store).map_err(|e| anyhow!("type_err: {e}"))?;
    let rc = typed.call(&mut store, ()).map_err(|e| anyhow!("call_err: {e}"))?;
